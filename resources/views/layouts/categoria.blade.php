...
<body>
    ...
    <x-filtro :subcategorias="$subcategorias" />
        ...
        <x-menu>
            <!-- Inicio -->
            <x-slot name="slot_inicio_texto">{{ $conoceMas['enlace_inicio']['texto'] ?? 'Inicio' }}</x-slot>
            <x-slot name="slot_inicio_url">{{ $conoceMas['enlace_inicio']['url'] ?? '#' }}</x-slot>

            <!-- Categorías -->
            <x-slot name="slot_categorias_titulo">{{ $categorias['titulo'] ?? 'Categorías' }}</x-slot>
            ...
            <!-- Comerciantes -->
            <x-slot name="slot_comerciantes_titulo">{{ $comerciantes['titulo'] ?? 'Comerciantes' }}</x-slot>
            ...
            <!-- Cómo funciona / Aprende -->
            <x-slot name="slot_como_funciona_titulo">{{ $aprende['titulo'] ?? 'Aprende' }}</x-slot>
            ...
            <!-- Contacto y Acciones -->
            <x-slot name="slot_contacto_texto">{{ $conoceMas['enlace_contacto']['texto'] ?? 'Contacto' }}</x-slot>
    ...
    <footer class="pt-5" style="background: #0A0A0A;">
        <div class="container px-4">
            <div class="row gy-4">
                            ...
                            @isset($generales['logo_empresa'])
                                <a href="{{ route('inicio') }}">
                                    <img src="{{ asset($generales['logo_empresa']) }}" 
                                         alt="Logo {{ $generales['nombre_empresa'] }}" 
                ...
                <!-- Categorías -->
                <div class="col-md-2">
                    <h6 class="fw-bold mb-3 text-white">{{ $categorias['titulo'] ?? 'Categorías' }}</h6>
                    <ul class="list-unstyled">
                        @foreach($categorias['items'] ?? [] as $item)
                            <li>
                ...
                <!-- Comerciantes -->
                <div class="col-md-2">
                    <h6 class="fw-bold mb-3 text-white">{{ $comerciantes['titulo'] ?? 'Comerciantes' }}</h6>
                    <ul class="list-unstyled">
                        @foreach($comerciantes['items'] ?? [] as $item)
                            <li>
                                <a href="{{ $item['url'] ?? '#' }}" class="text-white text-decoration-none link-hover">
                ...
                <!-- Aprende a usar -->
                <div class="col-md-2">
                    <h6 class="fw-bold mb-3 text-white">{{ $aprende['titulo'] ?? 'Aprende' }}</h6>
                    <ul class="list-unstyled">
                        @foreach($aprende['items'] ?? [] as $item)
                            <li>
                                <a href="{{ $item['url'] ?? '#' }}" class="text-white text-decoration-none link-hover">
